# Dependency notices

Tableloom code: Apache-2.0. Original starter artwork: CC0-1.0.

Fonts are bundled under the SIL Open Font License 1.1. Complete notices are in `font-licenses/`.

| Package | Version | Declared license |
| --- | --- | --- |
| @babel/helper-string-parser | 7.29.7 | MIT |
| @babel/helper-validator-identifier | 7.29.7 | MIT |
| @babel/parser | 7.29.8 | MIT |
| @babel/types | 7.29.8 | MIT |
| @fontsource/alegreya | 5.3.0 | OFL-1.1 |
| @fontsource/jetbrains-mono | 5.3.0 | OFL-1.1 |
| @fontsource/noto-sans-arabic | 5.3.0 | OFL-1.1 |
| @fontsource/noto-sans-hebrew | 5.3.0 | OFL-1.1 |
| @fontsource/source-sans-3 | 5.3.0 | OFL-1.1 |
| @fontsource/stardos-stencil | 5.3.0 | OFL-1.1 |
| @jridgewell/sourcemap-codec | 1.6.0 | MIT |
| @lucide/vue | 1.41.0 | ISC |
| @nodable/entities | 3.0.0 | MIT |
| @pdf-lib/fontkit | 1.1.1 | MIT |
| @pdf-lib/standard-fonts | 1.0.0 | MIT |
| @pdf-lib/upng | 1.0.1 | MIT |
| @vue/compiler-core | 3.5.42 | MIT |
| estree-walker | 2.0.2 | MIT |
| @vue/compiler-dom | 3.5.42 | MIT |
| @vue/compiler-sfc | 3.5.42 | MIT |
| estree-walker | 2.0.2 | MIT |
| magic-string | 0.30.21 | MIT |
| @vue/compiler-ssr | 3.5.42 | MIT |
| @vue/reactivity | 3.5.42 | MIT |
| @vue/runtime-core | 3.5.42 | MIT |
| @vue/runtime-dom | 3.5.42 | MIT |
| @vue/server-renderer | 3.5.42 | MIT |
| @vue/shared | 3.5.42 | MIT |
| anynum | 1.0.1 | MIT |
| csstype | 3.2.3 | MIT |
| entities | 7.0.1 | BSD-2-Clause |
| fast-xml-builder | 1.3.1 | MIT |
| fast-xml-parser | 5.11.1 | MIT |
| fflate | 0.8.3 | MIT |
| idb | 8.0.3 | ISC |
| is-unsafe | 2.0.2 | MIT |
| isomorphic.js | 0.2.5 | MIT |
| jsep | 1.4.0 | MIT |
| lib0 | 0.2.117 | MIT |
| nanoid | 3.3.18 | MIT |
| pako | 1.0.11 | (MIT AND Zlib) |
| papaparse | 5.7.0 | MIT |
| path-expression-matcher | 1.6.2 | MIT |
| pdf-lib | 1.17.1 | MIT |
| tslib | 1.14.1 | 0BSD |
| picocolors | 1.1.1 | ISC |
| postcss | 8.5.28 | MIT |
| source-map-js | 1.2.1 | BSD-3-Clause |
| strnum | 2.4.2 | MIT |
| typescript | 5.9.3 | Apache-2.0 |
| vue | 3.5.42 | MIT |
| ws | 8.21.3 | MIT |
| xml-naming | 0.3.0 | MIT |
| yjs | 13.6.32 | MIT |
| zod | 4.5.4 | MIT |

| semver | 7.x | ISC |
